...
export const Footer = () => {
  ...
  return (
    <footer className="relative">
      ...
      <div className="glass-darker rounded-t-3xl">
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              ...
              <div className="flex space-x-3">
                {shopInfo?.socialLinks?.map((link) => (
                  <a
                    ...
                    className="w-10 h-10 rounded-full flex items-center justify-center bg-white/8 border border-white/10 text-white/60 hover:text-white hover:bg-white/15 hover:border-white/20 transition-all duration-300"
            ...
            {/* Quick Links */}
            <div>
              <h3 className="text-lg font-semibold text-white/80 mb-4">Quick Links</h3>
              <ul className="space-y-2">
                {navigationItems.map((item) => (
                  <li key={item.path}>
                    <Link to={item.path} className="text-white/50 hover:text-white transition-colors duration-300 text-sm">{item.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {/* Contact Info */}
            <div>
              <h3 className="text-lg font-semibold text-white/80 mb-4">Contact Us</h3>
              {shopInfo && (
                <ul className="space-y-3 text-white/60 text-sm">
                  <li className="flex items-start space-x-2">
                    ...
                    <span>{shopInfo.address.street}, {shopInfo.address.city}, {shopInfo.address.state} {shopInfo.address.zipCode}</span>
                  </li>
            ...
            {/* Opening Hours */}
            <div>
              <h3 className="text-lg font-semibold text-white/80 mb-4">Opening Hours</h3>
              {shopInfo && (
                <ul className="space-y-2 text-white/60 text-sm">
                  {shopInfo.hours.slice(0, 5).map((hour) => (
                    <li key={hour.day} className="flex justify-between">
                      <span>{hour.day}</span>
                      <span className="text-white/80">{hour.open} - {hour.close}</span>
                    </li>
                  ))}
                  <li className="flex justify-between text-white/90 font-semibold pt-1 border-t border-white/10">
                    <span>Weekend</span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex space-x-6 mt-4 md:mt-0">
              ...
              <a href="#" className="text-white/40 hover:text-white/70 text-sm transition-colors duration-300">Terms of Service</a>
            </div>
          </div>
        </div>
      </div>
