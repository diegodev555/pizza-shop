...
export function AdminPromotions() {
  ...
  const [form, setForm] = useState({ title: '', description: '', discountText: '', imageUrl: '', code: '', validUntil: '' });
  ...
  function reset() { setForm({ title: '', description: '', discountText: '', imageUrl: '', code: '', validUntil: '' }); setEditingId(null); }
  ...
  function handleEdit(p: ApiPromotion) {
    setForm({ title: p.title, description: p.description, discountText: p.discountText, imageUrl: p.imageUrl || '', code: p.code || '', validUntil: p.validUntil || '' });
    setEditingId(p.id);
  }

  async function handleDelete(id: number) {
    if (!confirm('Delete this promotion?')) return;
    try { await adminApi.deletePromotion(id); setMessage({ type: 'success', text: 'Promotion deleted!' }); load(); } catch (err) { setMessage({ type: 'error', text: err instanceof Error ? err.message : 'Failed to delete' }); }
  }
